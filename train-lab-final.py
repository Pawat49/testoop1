...
class Account(ABC):
    ...
    def add_card(self, card):
        ...
        """
        if not (isinstance(card,ATM_Card) or isinstance(card,ShoppingCard) or isinstance(card,PremiumCard)):
            raise Exception("type False")
        self.__card = card
    ...
    def deposit(self, channel, amount):
        ...
        if not (isinstance(channel,ATM_machine) or isinstance(channel,EDC_machine) or isinstance(channel,Counter)):
            raise PermissionError("channel False")

        if amount <= 0:
            raise ValueError("amount False")
    ...
    def withdraw(self, channel, amount):
        ...
        """
        if not (isinstance(channel,ATM_machine) or isinstance(channel,EDC_machine)):
            raise PermissionError("channel False")
        
        if channel.current_card is None:
            raise PermissionError("not insert card")
...
class SavingAccount(Account):
    ...
    def _check_withdraw_limit(self, amount):
        ...
        if amount > self.WITHDRAW_LIMIT_PER_TRANSACTION:
            raise ValueError("withdraw False")
...
class FixedAccount(Account):
    ...
    def _check_withdraw_limit(self, amount):
        ...
        """
        maturity_date = self.__start_date + timedelta(days=self.__term_months*30)
        if datetime.now() < maturity_date:
            raise PermissionError("date False")
...
class ATM_machine(Channel):
    ...
    def insert_card(self, card, pin):
        ...
        if pin != card.pin:
            raise PermissionError("pin False")
        
        if self.__current_card is not None:
            raise PermissionError("have card in atm")
...
class Counter(Channel):
    ...
    def authenticate(self,citizen_id):
        pass
...
class Bank:
    ...
    def add_edc_machine(self, edc):
        ...
        if not isinstance(edc.account,CurrentAccount):
            raise Exception("type False")
...
class User:
    ...
    def add_account(self, account):
        ...
        if not (isinstance(account,SavingAccount) or isinstance(account,FixedAccount) or isinstance(account,CurrentAccount)):
            raise Exception("type False")
